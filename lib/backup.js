...
async function RecoverBackup(data, callback) {
    ...
    if (localFile == "") {
        ...
        localFile = backupFilePath + "/" + ret.data.name

        console.log("从服务器下载文件: ", data)
        let sret = await DownloadBackup("/api/backup/download", data, null, localFile)
        console.log("下载返回", sret)
        console.log("文件下载完成, 文件: ", localFile)
    }
...
function compareLocalFile(path, hash) {
    ...
    if (!fs.existsSync(path)) return "" // 目录不存在
